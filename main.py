...
ocupado = [170, 240, 260, 359, 259, 396, 372, 437, 450, 428, 534, 462, 554, 494, 452, 472, 475, 370, 452, 395, 299, 380, 314,
           200, 242, 248, 197, 370, 660, 621, 672, 747, 955, 909, 664, 705, 596, 244, 285, 684, 700, 908, 1082, 1239, 1027,
...
#Função para registrar o tempo durante a execução
def timer(segundos, lot):
    minutos = segundos // 60
    segundos = segundos % 60
    horas = minutos // 60
    minutos = minutos % 60
    cvzone.putTextRect(lot, "{0}:{1}:{2}".format(horas, minutos, segundos), (825, 45), scale=1.5, thickness=2, offset=0,
                       colorR=(188, 188, 188),
                       colorT=(0, 0, 0), font=cv.FONT_HERSHEY_DUPLEX)
...
def mainRun():
    ...
    while True:
        ...
        if not success:
            ...
            refMediano = cv.medianBlur(refBordas, 7)
            ...
            #Fornecimento da imagem para a página Web
            lot, buffer = cv.imencode('.jpg', lot)
            lot = buffer.tobytes()
            yield (b'--lot\r\n'
